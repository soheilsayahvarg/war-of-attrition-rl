...
class AblationShapingOnly(TheorySpec):
    """Beliefs from Sec. 4.3-4.4; decision statistics from Sec. 4.1-4.6."""

    name = "ablation-shaping-only"
    extra_feature_dim = 0
    ...
    def shaping(self, player_id, obs, action, env_reward, next_obs,
        ...
        leaves the optimal policy unchanged (Ng, Harada & Russell 1999), so
        it can only speed learning up, never redirect it away from the raw
        utility u_i that the leaderboard scores.
        """
        pid = player_id if player_id in ("I", "U") else "I"
        cur = self._potential(pid, obs)
        nxt = 0.0 if terminated else self._potential(pid, next_obs)
        return float(GAMMA * nxt - cur)

...
class EnduranceRaceTheory(TheorySpec):
    ...
    def shaping(self, player_id, obs, action, env_reward, next_obs,
        ...
        (2) THE PRICE OF CONCEDING. This one is *not* potential-based, and
        that is not an accident -- a potential term cannot express it,
        precisely because a potential term cannot move the optimum.

        The leaderboard does not score u_i, it ranks u_i against the field,
        and quitting is the one act that raises u_j directly: the opponent
        collects the whole outlasting prize B_j. So we charge quitting the
        size of that transfer, weighted by how much we believe the opponent
        is the kind that will actually be there to collect it:

            -CONCEDE_PRICE * P(commitment type) * E_posterior[ B_j ] / 175.

        Both factors matter, and each fixes a failure we measured.

        * The prize factor is largest *early*, when the opponent is fresh
          and B_j is near its ceiling. Our first attempt instead gated the
          charge on a detector with a threshold, and the policy simply
          learned to quit one stage *before* the detector could fire: the
          voluntary-exit rate stayed at 1.00 and the opponent's payoff went
          *up* (analysis/seat_split.py). A price on the transfer itself has
          no deadline to beat.

        * The posterior factor is what keeps this from becoming the "reward
          for merely surviving" the manual warns about in Sec. 5.2. Dropping
          it -- charging every concession alike -- does win the reference
          evaluation, but analysis/classpool.py shows it costs 20 points of
          our own U.S. score in a class-sized field, because the agent then
          also refuses to concede to opponents it simply cannot outlast and
          has no reason to deny. With the factor, a single non-mirroring
          reply drives P to zero and the charge disappears.

        It is charged on *both* seats. An earlier version charged the U.S.
        seat only, on the grounds of the asymmetric-race proposition: the
        U.S. side loses the symmetric attrition race at every signal level,
        so it is the seat the game structurally pushes into conceding. That
        restriction was really a way of bounding the damage back when the
        charge was unconditional. Once it is gated on the posterior it costs
        nothing against an opponent that is not a mirror, and the Iran seat
        hands away exactly the same prize when it concedes to one that is --
        which is the b' term of J_I. Charging both seats raised J_I from
        123.6 to 182.1 and left J_U healthy (analysis/seat_split.py).
        """
        pid = player_id if player_id in ("I", "U") else "I"

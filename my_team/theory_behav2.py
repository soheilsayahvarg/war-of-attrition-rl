...
class EnduranceRaceTheory(TheorySpec):
    ...
    name = "endurance-race-filter+behaviour+evidence"
...
# enough. The U.S. seat scored 113.3 against Random by pinning
        ...
        # How much evidence those statistics rest on. This is the feature
        # the first attempt was missing, and the omission is exactly why it
        # failed. sd = 0 means "this opponent never varies" after eight
        # observations and "we have seen one move" after one; max_sig = 1
        # with a single observation is equally consistent with Hawk (always
...
# analysis/seat_probe.py caught the consequence. The U.S. seat
        ...
        #     final_v4    113.3     0.677   4.8     0.10       0.90
        #     bh_s2        64.1     0.000   2.3     0.70       0.33
        #
        # The 19-feature policy quits 70% of the time at stage ~2 -- before
        # any of these statistics can separate anything -- and forfeits a
        # prize Random was about to hand over. It is applying the correct
        # response to the wrong opponent, because at stage 2 the two look
        # identical. Saturating at 8 stages: past that the sample statistics
        # stand on their own.
        n_free = max(int(st["n_free"]), 0)
        n_evidence = min(n_free, 8) / 8.0
        if n_free > 0:
        ...
        f = np.array([
            ...
            n_evidence,                     # 19 how much those rest on
        ], dtype=np.float32)
